...
# fit_ols(train, imputed_fits$c1_fit$coef)
# 
# 
# t <- train %>% select(class, n_quit_attempts) %>% mutate(n_quit_attempts = relevel(n_quit_attempts, ref = "2 TO 5 TIMES"))
# 
# lm(class ~ ., data = t)
# 
# 
# t <- train %>% filter(n_quit_attempts_NEVER == 1)
